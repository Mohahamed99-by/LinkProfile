                ...
                <div className="mb-8 lg:mb-0">
                  ...
                  <div className="flex flex-wrap gap-4">
                    <button
                      ...
                      className="group inline-flex items-center px-6 py-3 bg-white/80 border border-slate-200 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 hover:-translate-y-1"
                    >
      ...
      {/* Quick Overview Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
          <div className="mb-4 sm:mb-0">
            ...
            <p className="text-lg text-slate-600">Your essential metrics at a glance</p>
          </div>
          
          <div className="flex items-center space-x-3">
            <Link
              to="/analytics"
              className="group inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 hover:-translate-y-1"
            >
...
const StatCard = ({ icon: Icon, title, value, gradient, description, isText = false, loading = false, growth = null }) => (
  <div className="group relative glass-card rounded-3xl p-6 border border-white/60 shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-105 hover:-translate-y-2">
    ...
    {/* En-tête avec icône */}
    <div className="flex items-center justify-between mb-6">
      <div className={`relative w-16 h-16 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-6`}>
        <Icon className="text-white text-2xl" />
        
        {/* Points décoratifs */}
        <div className="absolute -top-1 -right-1 w-3 h-3 bg-white/30 rounded-full opacity-60 group-hover:opacity-100 transition-opacity duration-300"></div>
      </div>
      ...
      {growth !== null ? (
        ...
        <div className="w-2 h-2 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full opacity-60 group-hover:opacity-100 group-hover:scale-150 transition-all duration-300"></div>
      )}
    </div>
    ...
    <div className="relative z-10">
      ...
      {loading ? (
        <div className="h-10 bg-gradient-to-r from-slate-200 to-slate-300 rounded-xl animate-pulse mb-2"></div>
      ) : (
        <p className="text-3xl lg:text-4xl font-black bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent mb-2 group-hover:scale-110 transition-transform duration-300">
    ...
    {/* Bordure animée */}
    <div className={`absolute inset-0 rounded-3xl bg-gradient-to-br ${gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-500 -z-10 blur-xl`}></div>
  </div>
)

// Composant LinkCard Moderne
const LinkCard = ({ link, index, onDelete }) => (
  <div className="group relative bg-white/80 backdrop-blur-sm border border-white/60 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1">
    {/* Effet de brillance */}
    ...
    <div className="flex items-center justify-between relative z-10">
      <div className="flex items-center space-x-4">
        {/* Icône de plateforme */}
        <div className="relative">
          <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
            <i className={`${link.icon} text-white text-xl`}></i>
          </div>
          {/* Point décoratif */}
          <div className="absolute -top-1 -right-1 w-4 h-4 bg-gradient-to-br from-emerald-400 to-teal-400 rounded-full opacity-60 group-hover:opacity-100 transition-opacity duration-300"></div>
        </div>
        
        {/* Informations du lien */}
        <div className="flex-1">
          <h3 className="text-lg font-bold text-slate-800 group-hover:text-blue-600 transition-colors duration-300 mb-1">
            {link.platform_name}
          </h3>
          ...
          {/* Badge de statut */}
          <div className="inline-flex items-center mt-2 px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-semibold">
      ...
      <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
        <a
          ...
          className="p-3 text-slate-400 hover:text-blue-600 transition-all duration-300 rounded-xl hover:bg-blue-50 hover:scale-110"
          title="Visit Link"
        >
        ...
        <Link
          ...
          className="p-3 text-slate-400 hover:text-emerald-600 transition-all duration-300 rounded-xl hover:bg-emerald-50 hover:scale-110"
          title="Edit Link"
        >
        ...
        <button
          ...
          className="p-3 text-slate-400 hover:text-red-600 transition-all duration-300 rounded-xl hover:bg-red-50 hover:scale-110"
          title="Delete Link"
        >
...
const EmptyState = () => (
  <div className="text-center py-20">
    ...
    <div className="relative mb-8">
      <div className="w-32 h-32 bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100 rounded-3xl flex items-center justify-center mx-auto shadow-lg">
        <div className="relative">
          <FaLink className="text-blue-500 text-4xl" />
          {/* Éléments décoratifs */}
          <div className="absolute -top-2 -right-2 w-6 h-6 bg-gradient-to-br from-emerald-400 to-teal-400 rounded-full flex items-center justify-center">
      ...
      <div className="absolute top-4 left-1/2 transform -translate-x-8 w-3 h-3 bg-blue-400 rounded-full animate-ping"></div>
    ...
    {/* Contenu */}
    <h3 className="text-2xl lg:text-3xl font-black text-slate-800 mb-4">
